 ...
 */
@Autonomous(name="CF_Far_Beacon_Blue", group ="Blue")
//@Disabled
public class CF_Far_Beacon_Blue extends CF_Library{
    // Position
    OpenGLMatrix pose;
        ...
        while(opModeIsActive() && !breakLoop && runtime.seconds() < endTime) {
            switch(State) {
                case FIRSTSTRAFE:
                    ...
                    this.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
                    this.encoderStrafeRight(1000, speed);
                    this.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
                    //this.encoderMove(6600, 6600, 1.0f, 1.0f);
                    this.encoderMove(6750, 6750, 0.9f, 0.9f); //0.9
                    this.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
                    this.encoderMove(2150, -2150, 0.5f, 0.5f);
